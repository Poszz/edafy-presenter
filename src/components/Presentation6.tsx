...
import { CoverSlide } from './slides6/CoverSlide';
import { TitleSlide } from './slides6/TitleSlide';
import { AboutSlide } from './slides6/AboutSlide';
import { ChallengeSlide } from './slides6/ChallengeSlide';
import { SolutionsSlide } from './slides6/SolutionsSlide';
import { CapabilitiesSlide } from './slides6/CapabilitiesSlide';
...
interface Presentation6Props {
  onBack: () => void;
}

export function Presentation6({ onBack }: Presentation6Props) {
  ...
  return (
    <main className="w-full h-screen overflow-hidden relative bg-transparent">
      {/* Logo — top left */}
      ...
      <div className="nav-dots fixed right-6 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-3">
        ...
        <button
          ...
          className={`w-3 h-3 rounded-full transition-all duration-300 ${activeSlide === idx ? 'bg-white scale-125 shadow-[0_0_15px_rgba(168,85,247,0.8)]' : 'bg-white/30 hover:bg-white/60'}`} />
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 flex items-center justify-between px-8 py-4">
        <button
          ...
          className={`group flex items-center gap-2 px-5 py-3 rounded-full bg-white/10 backdrop-blur-xl border border-white/20 shadow-lg transition-all duration-300 ${activeSlide === 0 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-white/20 hover:scale-105 hover:shadow-[0_0_20px_rgba(168,85,247,0.4)] cursor-pointer'}`}>
        ...
        <div className="flex items-center gap-3 px-5 py-3 rounded-full bg-white/10 backdrop-blur-xl border border-white/20 shadow-lg">
          <span className="text-white font-bold text-sm drop-shadow-[0_0_8px_rgba(168,85,247,0.8)]">
        ...
        <button
          ...
          className={`group flex items-center gap-2 px-5 py-3 rounded-full bg-white/10 backdrop-blur-xl border border-white/20 shadow-lg transition-all duration-300 ${activeSlide === totalSlides - 1 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-white/20 hover:scale-105 hover:shadow-[0_0_20px_rgba(168,85,247,0.4)] cursor-pointer'}`}>
